...
public class SellerRepositoryImplMySQL implements SellerRepository {
    ...
    public Seller getSellerByVatAndSerial(){
        try {
            ...
            String sql = "SELECT SEQ_ID, ID_NUMBER, ID_TYPE, VAT_NUMBER, NAME_E, NAME_A, BUILDING_NO, STREET, LINE, DISTRICT, ADDITIONAL_NO, POSTAL_CD, CITY, COUNTRY, CNAME, SERIAL_NO, LOCATION, INDUSTRY, INV_TYPES, OUNIT, CSR, PRIVATE_KEY, REQUEST_ID_COMP, CERT_COMP, KEY_COMP, CERT_PROD, KEY_PROD, REQUEST_ID_PROD, SLR_STS, CERT_STS, ZATCA_FLG, IMAGE  FROM invslr LIMIT 1;";
    ...
    public void update(Seller seller) {
        ...
        try
        {
            String sql = "UPDATE invslr" +
                    ...
                    " INV_TYPES = ? , OUNIT = ? , CSR = ? , PRIVATE_KEY = ? , REQUEST_ID_COMP = ? , CERT_COMP = ? , KEY_COMP = ? , SLR_STS = ? ,CERT_PROD = ? , KEY_PROD = ? , REQUEST_ID_PROD = ? , CERT_STS = ?  ,ZATCA_FLG = ?, UPD_DT = now(), IMAGE = ? WHERE VAT_NUMBER = ? AND SERIAL_NO = ? ;";
    ...
    public LocalDate getExpiryDate()
    {
        ...
        try
        {
            String sql = "SELECT EXPIRY_DT FROM SELLER_INFO LIMIT 1;";
